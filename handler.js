...
export async function handler(chatUpdate) {
    ...
    global.img = 'https://telegra.ph/file/5b51211f2fcfb75a15057.jpg' 
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `[❗] Limitmu udah abis dek, beli melalui *${usedPrefix}buy limit*`, m)
                    ...
                    if (e) {
                        ...
                        if (e.name)
                            for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                                let data = (await conn.onWhatsApp(jid))[0] || {}
                                if (data.exists)
                                    m.reply(`*🗿Hai Owner Fitur Mu Ada Yang Eror Tuh Buru Di Fix*\n *🗂️ Plugins:* ${m.plugin}\n*👤 Dikirim Oleh:* ${m.sender}\n*💬 Chat:* ${m.chat}\n*💻 Perintah:* ${usedPrefix}${command} ${args.join(' ')}\n📄 *Catatan Error:*\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                        m.reply(text)
                    }
                } finally {
...
export async function participantsUpdate({ id, participants, action }) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    let pp = 'https://telegra.ph/file/5b51211f2fcfb75a15057.jpg'
                    try {
 ...
 /* conn.sendButtonDoc(id, wm, text, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'Aʟᴅɪ X Aɪsʏᴀʜ', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
  ...
  conn.sendButtonImg(id, await(await fetch(action === 'add' ? wel : lea)).buffer(), 'Group Messege', text, action == 'add' ? 'Selamat Datang 😇' : 'Sampai Jumpa 👋', action === 'add' ? '.intro' : 'heheh 🗿', fkontak, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'https://wahyudiid.w3spaces.com',
    mediaType: 2, 
    description: sgc,
    title: "Jᴏɪɴ Sɪɴɪ Cᴜʏ",
    body: bottime,
...
function ucapan() {
  ...
  if (time >= 10) {
    res = "Selamat Siang Senpai ☀️"
  }
  if (time >= 15) {
    res = "Selamat Sore Senpai 🌇"
  }
  if (time >= 18) {
    res = "Malam Senpai 🌙"
  }
